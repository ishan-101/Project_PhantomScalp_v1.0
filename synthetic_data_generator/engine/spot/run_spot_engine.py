# synthetic_data_generator/engine/spot/run_spot_engine.py
...
def _resolve_run_engine():
    candidates = [
        "synthetic_data_generator.engine.spot.spot_engine",
        "engine.spot.spot_engine",
    ]
    for module_path in candidates:
        ...
        if spec is not None:
            ...
            if callable(run_engine):
                return run_engine
    file_path = Path(__file__).resolve().parent / "spot_engine.py"
    spec = importlib.util.spec_from_file_location("spot_engine_direct", file_path)
    if spec is None or spec.loader is None:
        raise ImportError("Unable to resolve spot_engine module for runner.")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    run_engine = getattr(module, "run_engine", None)
    if callable(run_engine):
        return run_engine
    raise ImportError("run_engine not found in spot_engine module.")
...
# Minimal schema contract for spot output (matches engine columns/dtypes)
SPOT_SCHEMA_SPEC: Dict[str, Any] = {
    "required_columns": {
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run the spot engine")
    parser.add_argument("--rows", type=int, help="Override row count for this run")
    parser.add_argument("--start_ts", type=str, help="Override start timestamp (ISO, will be forced to UTC)")
    ...
    print("[runner] starting spot engine with arguments:")
    ...
    print("[runner] spot engine completed")
    ...
    schema_spec = dict(SPOT_SCHEMA_SPEC)
