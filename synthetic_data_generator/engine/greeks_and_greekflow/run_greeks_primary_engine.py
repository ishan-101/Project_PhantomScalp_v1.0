...
def _resolve_run_engine():
    candidates = [
        "synthetic_data_generator.engine.greeks_and_greekflow.greeks_primary_engine",
        "engine.greeks_and_greekflow.greeks_primary_engine",
    ]
    for module_path in candidates:
        ...
        if spec is not None:
            ...
            if callable(run_engine):
                return run_engine
    file_path = Path(__file__).resolve().parent / "greeks_primary_engine.py"
    spec = importlib.util.spec_from_file_location("greeks_primary_engine_direct", file_path)
    if spec is None or spec.loader is None:
        raise ImportError("Unable to resolve greeks_primary_engine module for runner.")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    run_engine = getattr(module, "run_engine", None)
    if callable(run_engine):
        return run_engine
    raise ImportError("run_engine not found in greeks_primary_engine module.")
...
GREEKS_PRIMARY_SCHEMA_SPEC: Dict[str, Any] = {
    "required_columns": {
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run the Greeks Primary engine")
    parser.add_argument("--rows", type=int, help="Override row count for this run")
    parser.add_argument("--symbol", type=str, help="Override symbol (e.g., BTC-202601-C)")
    parser.add_argument("--exchange", type=str, help="Override exchange code")
    ...
    print("[runner] starting greeks primary engine with arguments:")
    print(json.dumps(filtered_kwargs, indent=2))

    result = run_engine(**filtered_kwargs)
    print("[runner] greeks primary engine completed")
    ...
    schema_spec = dict(GREEKS_PRIMARY_SCHEMA_SPEC)
    ...
    manifest_entry = manifest_mod.build_manifest_entry(
        engine="greeks_primary",
        dataset_type="greeks_primary",
    ...
    provenance_record = prov.build_provenance_record(
        dataset_name="greeks_primary",
        engine_name="greeks_primary_engine",
