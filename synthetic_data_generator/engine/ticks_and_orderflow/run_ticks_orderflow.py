#!/usr/bin/env python3
"""
Runner for ticks_orderflow engine — with post-write validation, manifest, and provenance.
Usage:
...
try:
    from synthetic_data_generator.engine.ticks_and_orderflow.ticks_orderflow_engine import run_engine
except Exception:
    from engine.ticks_and_orderflow.ticks_orderflow_engine import run_engine
...
TICKS_ORDERFLOW_SCHEMA_SPEC: Dict[str, Any] = {
    "required_columns": {
...
def main():
    try:
        ...
        schema_spec = dict(TICKS_ORDERFLOW_SCHEMA_SPEC)
        ...
        manifest_entry = manifest_mod.build_manifest_entry(
            engine="ticks_orderflow",
            dataset_type="ticks_orderflow",
        ...
        provenance_record = prov.build_provenance_record(
            dataset_name="ticks_orderflow",
            engine_name="ticks_orderflow_engine",
