# synthetic_data_generator/engine/options/run_options_oi.py
...
def _resolve_run_engine():
    candidates = [
        "synthetic_data_generator.engine.options.options_oi_engine",
        "engine.options.options_oi_engine",
    ]
    for module_path in candidates:
        ...
        if spec is not None:
            ...
            if callable(run_engine):
                return run_engine
    file_path = Path(__file__).resolve().parent / "options_oi_engine.py"
    spec = importlib.util.spec_from_file_location("options_oi_engine_direct", file_path)
    if spec is None or spec.loader is None:
        raise ImportError("Unable to resolve options_oi_engine module for runner.")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    run_engine = getattr(module, "run_engine", None)
    if callable(run_engine):
        return run_engine
    raise ImportError("run_engine not found in options_oi_engine module.")
...
OPTIONS_OI_SCHEMA_SPEC: Dict[str, Any] = {
    "required_columns": {
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run the options OI engine")
    parser.add_argument("--rows", type=int, help="Override row count for this run")
    parser.add_argument("--start_ts", type=str, help="Override start timestamp (ISO, will be coerced to UTC)")
    ...
    kwargs = {"rows": args.rows, "start_ts": args.start_ts, "exchange": args.exchange, "symbol": args.symbol}

    print("[runner] starting options OI engine with arguments:")
    ...
    print("[runner] options OI engine completed")
    ...
    schema_spec = dict(OPTIONS_OI_SCHEMA_SPEC)
    ...
    provenance_record = prov.build_provenance_record(
        dataset_name="options_oi",
        engine_name="options_oi_engine",
