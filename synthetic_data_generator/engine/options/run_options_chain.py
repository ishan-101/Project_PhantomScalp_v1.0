# synthetic_data_generator/engine/options/run_options_chain.py
...
def _resolve_run_engine():
    candidates = [
        "synthetic_data_generator.engine.options.options_chain_engine",
        "engine.options.options_chain_engine",
    ]
    for module_path in candidates:
        ...
        if spec is not None:
            ...
            if callable(run_engine):
                return run_engine
    file_path = Path(__file__).resolve().parent / "options_chain_engine.py"
    spec = importlib.util.spec_from_file_location("options_chain_engine_direct", file_path)
    if spec is None or spec.loader is None:
        raise ImportError("Unable to resolve options_chain_engine module for runner.")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    run_engine = getattr(module, "run_engine", None)
    if callable(run_engine):
        return run_engine
    raise ImportError("run_engine not found in options_chain_engine module.")
...
OPTIONS_CHAIN_SCHEMA_SPEC: Dict[str, Any] = {
    "required_columns": {
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run the options chain engine")
    ...
    print("[runner] starting options_chain engine with arguments:")
    ...
    print("[runner] options_chain engine completed")
    ...
    schema_spec = dict(OPTIONS_CHAIN_SCHEMA_SPEC)
    ...
    provenance_record = prov.build_provenance_record(
        dataset_name="options_chain",
        engine_name="options_chain_engine",
