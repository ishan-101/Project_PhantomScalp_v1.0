...
def _generate_l1_snapshots(
    ...
    inter_arrival = np.random.exponential(scale=AVG_INTERVAL_SECONDS, size=n)
    ...
    numeric_cols = [
        c
        for c in df.columns
        if c not in ("meta__timestamp", "exchange", "symbol", "meta__sequence_id")
    ]
...
def run_engine(
    ...
    while produced < n_total:
        ...
        chunk_start_ts = start_ts + pd.to_timedelta(produced * AVG_INTERVAL_SECONDS, unit="s")
        df = _generate_l1_snapshots(
            RNG,
            Seq,
            this_n,
            chunk_start_ts,
