# run_orderbook_l2.py
"""
Runner for orderbook_l2 with post-write validation, manifest, and provenance.
"""
import importlib
...
ENGINE_MODULE_CANDIDATES = [
    "synthetic_data_generator.engine.orderbook.orderbook_l2_engine",
    "engine.orderbook.orderbook_l2_engine",
]
...
for _module_path in ENGINE_MODULE_CANDIDATES:
    ...
    raise ImportError("Unable to import orderbook_l2_engine.run_engine")
...
ORDERBOOK_L2_SCHEMA_SPEC: Dict[str, Any] = {
    "required_columns": {
...
def _build_engine_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rel_path = cfg.get("paths", {}).get("orderbook_l2", "orderbook/l2")
    out_path = base_path / rel_path / "orderbook_l2.parquet"
    ...
    default_batch = sharder.get("max_rows_per_file") or sharder.get("min_rows_per_file")

    engine_defaults = getattr(engine_module, "DEFAULT_CONFIG", {})

    return {
        **engine_defaults,
        "rows": int(cfg.get("rows", {}).get("orderbook_l2", engine_defaults.get("rows", 0))),
        "batch_size": int(default_batch or engine_defaults.get("batch_size", 20000)),
...
if __name__ == "__main__":
    print("[runner] Starting orderbook_l2 engine...")
    ...
    schema_spec = dict(ORDERBOOK_L2_SCHEMA_SPEC)
    ...
    provenance_record = prov.build_provenance_record(
        dataset_name="orderbook_l2",
        engine_name="orderbook_l2_engine",
